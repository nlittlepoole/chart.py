...
class PieChart():
    ...
    def build_chart(self):
        ...
        return chart.replace("myLineChart", "myLineChart%d" % token ).replace("ctx", "ctx%d" % token ).replace("kanvas", "kanvas%d" % token ).replace("myChart", "myChart%d" % token ).replace("change", "change%d" % token ).replace("data", "data%d" % token )
    def build_html(self):
